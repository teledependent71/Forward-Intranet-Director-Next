...
import authorsPageInitialProps88d5cResource from '../../resources/authors-page-initial-props-88d5c'
  ...
  return (
    <>
      <div className="authors1-container">
        <Head>
          <title>Authors1 - Forward Intranet Director</title>
          <meta
            property="og:title"
            content="Authors1 - Forward Intranet Director"
          />
        </Head>
        <DataProvider
...
export async function getStaticProps(context) {
  try {
    const response = await authorsPageInitialProps88d5cResource({
      ...context?.params,
    })
    if (!response) {
      return {
